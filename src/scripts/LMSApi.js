...
API.LMSInitialize = function (dummyString) {
    //alert('init');
...
API.LMSGetValue = function (varname) {

    // not initialized or already finished
    if ((!flagInitialized) || (flagFinished)) { return "false"; }

    // otherwise, return the requested dat
    //alert(cache[varname]);
...
API.LMSCommit = function (dummyString) {
    ...
    req.open('POST', '/Pages/LMS/Commit', false);

    // create a POST-formatted list of cached data elements 
    // include only SCO-writeable data elements
    var params = "&code=" + d.getTime();
    params += "&data[id_course]=" + urlencode(cache['id_course']);
    params += "&data[id_user]=" + urlencode(cache['id_user']);
...
API.LMSFinish = function (dummyString) {
    // not initialized or already finished
    if ((!flagInitialized) || (flagFinished)) { return "false"; }
    ...
    // set up request parameters - uses POST method
    req.open('POST', '/Pages/LMS/Finish', false);
